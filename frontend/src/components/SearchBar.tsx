...
const SearchBar=()=>{
    ...
    const [checkIn,setCheckIn]=useState<Date>(search.checkOut);
    ...
    const handleSubmit = (event: FormEvent) => {
      ...
      return (
        <form onSubmit={handleSubmit}
          className="-mt-8 p-3 bg-orange-400 shadow-md grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 items-center gap-4 rounded">
            ...
            <div className="flex bg-white px-2 py-1 gap-2 rounded">
                <label className="items-center flex"> Adults:
                    <input className="w-full p-1 focus:outline-none font-bold"
                        type="number" min={1} max={20} value={adultCount}
                        onChange={(event) => setAdultCount(parseInt(event.target.value))}
                    />
                </label>
                <label className="items-center flex"> Children:
                    <input className="w-full p-1 focus:outline-none font-bold"
                        type="number" min={0} max={20} value={childCount}
