...
test("should edit hotel", async ({ page }) => {
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("MyHotel");

  await page.locator('[name="name"]').fill("MyHotel Updated");
  await page.getByRole("button", { name: "Save" }).click();

  await expect(page.getByText("Hotel Updated!")).toBeVisible({timeout:10000});
  ...
  await page.locator('[name="name"]').fill("MyHotel");
  await page.getByRole("button", { name: "Save" }).click();
});